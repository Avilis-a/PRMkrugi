...
maxCoordX = 600
maxCoordY = 700
mas = []  # массив для задания препятствий
obstacles = []  # массив для хранения препятсвий
start = []  # начальная вершина
end = []    # конечная вершина


# поиск ближайшей вершины в graph к currentNode
...
# поиск вершин в graph, которые находятся на расстоянии dist от currentNode
...
# определяет пересекаются ли два отрезка заданные точками начала и конца
...
# проверка пересекает ли отрезок ограниченный node1 и node2 какие-либо препятствия из obstacles
def Colides(obstacles, node1, node2):
    for obstacle in obstacles:
        ...
        node = ((node1[0] + t * (node1[0] - node2[0])), (node1[1] +
                                                         t * (node1[1] - node2[1])))
        # проверка на пересечение прямой node1 node2 и прямой из центра окружности до node
...
def PRM(maxIt, k):
    ...
    # проверка на то, что какая-то из заданных вершин нахожится в препятствии
    for obstacle in obstacles:
    ...
    # добавление в граф maxIt новых вершин
    for _ in range(maxIt):
        currentNode = (random.randint(10, maxCoordX - 10),
                       random.randint(10, maxCoordY - 10))

        # проверка вершины чтобы она не оказалась в препятствии
        for _ in range(2):
            for obstacle in obstacles:
    ...
    # соединение всех вершин с вершинами находящимися на расстоянии меньше чем k
    # флаги fSt и fEn показывают были ли соединены вершины start и end соответственно
    # с какими-нибудь другими вершинами
    fSt = False
    fEn = False
    for node in g.nodes():
...
# вычисление расстояния между точками node1 и node2
...
def dijkstra_algorithm(graph, start_node):
    ...
    # массив с расстояниями от start_node до каждой вершины графа
    shortest_path = {}

    # массив с предыдущими вершинами в пути для каждой вершины
    previous_nodes = {}
...
# получение пути от start_node до target_node путем прохождения от target до start
# по массиву previous_nodes, затем полученный путь разворачиваем и получаем искомый
...
# обработка нажатия на кнопку draw edges
def draw_edges():
    ...
    if graph is None:
        ...
        if shortest_path[(end[0], end[1])] == sys.maxsize:
            # проверка если нет маршрута до end,
            # то есть соответствующее значени в словаре равно maxsize
            mb.showinfo("Маршрут", "нет пути")  # вывод сообщения
        else:
            path = get_result(
                previous_nodes, (start[0], start[1]), (end[0], end[1]))
            for u, v in graph.edges():
                canvas.create_line(u[0], u[1], v[0], v[1], tags="cc")  # создание линии из точки u в точку v
                canvas.create_oval(u[0] - 3, u[1] - 3, u[0] +
                                   3, u[1] + 3, fill="yellow", tags="cc")  # создание окружности (обозначает точки)
            ...
            # выделение полученного кратчайшего пути
...
# обработка нажатия на кнопку save scene
def save_scene():
    # открывается файл и в него записываются все значения
    file = open("PRM.txt", "w")
...
# обработка нажатия на кнопку load scene
def load_scene():
    # открывается файл и из него считываются данные
    # так как считывание идет по формату, то если изначально была сохранена не вся сцена, 
    # то есть не было начальной или конечной вершин или препятсвий то не считается, будет ошибка
    file = open("PRM.txt")
    canvas.delete("all")
    obstacles.clear()
    start.clear()
...
# обработка нажатия на кнопку clear scene
def clear_scene():
...
# создание холста (где рисуется граф и препятствия)
...
# обрабока нажатия на ЛКМ
def add_obstacle(event):
    mas.append((event.x, event.y))

    # как только получены две точки: радиус и центр, создаем круг и очищаем массив
    if len(mas) == 2:
...
# присвоение функций к нажатиям на холст
